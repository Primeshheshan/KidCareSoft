...
export class StudentService {
  ...
  async create(student: StudentCreateDto): Promise<Student> {
    const studentDetial: Student = this.studentRepository.create(student);
    return await this.studentRepository.save(studentDetial);
  }

  async delete(id: string): Promise<Student> {
    const existingStudnet: Student = await this.studentRepository.findOneBy({
      id,
    });
  ...
  async update(
    ...
    const newStudent: Student = this.studentRepository.create(student);
    newStudent.id = id;

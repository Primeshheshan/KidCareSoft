...
export class AuthService {
  ...
  async refreshToken(email: string, refreshToken: string) {
    ...
    if (user.length === 0 && !user[0].hashedRefreshToken)
      throw new ForbiddenException('Access Denied');

    if (refreshToken !== user[0].hashedRefreshToken)
      throw new ForbiddenException('Access Token Mismatched or Already logout');
